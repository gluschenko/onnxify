...
internal sealed class ConvTest : Operator
{
    ...
    public static ConvTest FromNodeProto(NodeProto nodeProto)
    {
        ...
        var op = new ConvTest
        {
            ...
            InputB = inputB != null ? new FormalParameter<TensorProto>("B", inputB) : null,
        ...
        foreach (var attr in nodeProto.Attribute)
        {
            ...
            {
                case "auto_pad":
                    op.AttributeAuto_pad = (OperatorAttribute<string>)OperatorHelpers.MapAttributeType(attr);
                    break;
                case "dilations":
                    op.AttributeDilations = (OperatorAttribute<long[]>)OperatorHelpers.MapAttributeType(attr);
                    break;
                case "group":
                    op.AttributeGroup = (OperatorAttribute<long>)OperatorHelpers.MapAttributeType(attr);
                    break;
                case "kernel_shape":
                    op.AttributeKernel_shape = (OperatorAttribute<long[]>)OperatorHelpers.MapAttributeType(attr);
                    break;
                case "pads":
                    op.AttributePads = (OperatorAttribute<long[]>)OperatorHelpers.MapAttributeType(attr);
                    break;
                case "strides":
                    op.AttributeStrides = (OperatorAttribute<long[]>)OperatorHelpers.MapAttributeType(attr);
                    break;
                default:
                    throw new NotImplementedException($"Not implemented for '{attr.Name}'");
            }
        }
