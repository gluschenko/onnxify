...
internal sealed class PackageInventory
{
    ...
    public string BuildFullMarkdown()
    {
        var builder = new StringBuilder();
        builder.AppendLine("# Onnxify Package Versions And Dependencies");
        builder.AppendLine();
    ...
    public string BuildReferenceLinkMarkdown(string relativePath)
    {
        var builder = new StringBuilder();
        builder.AppendLine("## Package Versions");
        builder.AppendLine();
        builder.Append("[Full package versions and dependencies](")
            .Append(relativePath.Replace('\\', '/'))
            .AppendLine(").");

        return builder.ToString().TrimEnd();
    }

    public string BuildPackageVersionMarkdown(
        IReadOnlyList<string> packageNames,
        string fullInventoryRelativePath
    )
    {
        var selectedPackages = Packages
            .Where(package => packageNames.Contains(package.Name, StringComparer.Ordinal))
        ...
        builder.AppendLine();
        builder.Append("[Full package versions and dependencies](")
            .Append(fullInventoryRelativePath.Replace('\\', '/'))
            .AppendLine(").");
        builder.AppendLine();
        builder.AppendLine("| Package | Version |");
        builder.AppendLine("| --- | --- |");

        foreach (PackageDoc package in selectedPackages)
        {
            builder.Append("| `")
                .Append(EscapeMarkdownCell(package.Name))
                .Append("` | `")
