...
- name the primary C# runtime type `Safetensors` in PascalCase; do not spell it as `SafeTensors`
...
4. Dtype coverage using existing Onnxify numeric primitives where available.
...
- Keep the primary runtime type name fixed as `Safetensors`. This should be treated as a naming rule, not just a preference, so the public API does not drift between `Safetensors` and `SafeTensors`.
