...
- Input options type: `ReduceMaxInputOptions`
- Input/output options type: `ReduceMaxInputOutputOptions`
- Wrapper overloads detected via reflection:
  - `graph.ReduceMax(string name, ReduceMaxInputOptions options) -> IOnnxGraphEdge`
  - `graph.ReduceMax(string name, ReduceMaxInputOutputOptions options) -> IOnnxGraphEdge`
...
| Converter | Torch ops |
| --- | --- |
| `Onnxify.TorchSharp.TorchTensorOperatorExtensions.ExportAMax(this OnnxGraph graph, IOnnxGraphEdge input, IReadOnlyList<long> dims, bool keepdim) -> IOnnxGraphEdge` | `aten::amax` |
