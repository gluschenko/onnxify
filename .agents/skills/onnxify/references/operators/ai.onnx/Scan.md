...
- Input options type: `ScanInputOptions`
- Input/output options type: `ScanInputOutputOptions`
- Wrapper overloads detected via reflection:
  - `graph.Scan(string name, ScanInputOutputOptions options) -> IReadOnlyList<IOnnxGraphEdge>`
...
| `num_scan_inputs` | `NumScanInputs` | `long` | yes | `[null]` | An attribute specifying the number of scan_inputs M. |
| `scan_input_axes` | `ScanInputAxes` | `long[]` | no | `[null]` | An optional list of M flags. The i-th element of the list specifies the axis to be scanned (the sequence axis) for the i-th scan_input. If omitted, 0 will be used as the scan axis for every scan_input. Negative value for an axis means counting dimensions from the back. Accepted range is [-r, r-1] where r = rank(input). |
| `scan_input_directions` | `ScanInputDirections` | `long[]` | no | `[null]` | An optional list of M flags. The i-th element of the list specifies the direction to be scanned for the i-th scan_input tensor: 0 indicates forward direction and 1 indicates reverse direction. If omitted, all scan_input tensors will be scanned in the forward direction. |
| `scan_output_axes` | `ScanOutputAxes` | `long[]` | no | `[null]` | An optional list of K flags. The i-th element of the list specifies the axis for the i-th scan_output. The scan outputs are accumulated along the specified axis. If omitted, 0 will be used as the scan axis for every scan_output. Negative value for an axis means counting dimensions from the back. Accepted range is [-r, r-1]. |
| `scan_output_directions` | `ScanOutputDirections` | `long[]` | no | `[null]` | An optional list of K flags, one for each scan_output. The i-th element of the list specifies whether the i-th scan_output should be constructed by appending or prepending a new value in each iteration: 0 indicates appending and 1 indicates prepending. If omitted, all scan_output tensors will be produced by appending a value in each iteration. |
