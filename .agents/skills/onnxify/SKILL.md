...
- `Onnxify.ML` (`src/Onnxify.ML`): a publishable package slot reserved by the repo, but the current code surface is only a placeholder. Treat it as early-stage and verify the actual source before assuming meaningful API coverage.
- `Onnxify.ML.TorchSharp` (`src/Onnxify.ML.TorchSharp`): a publishable TorchSharp-oriented ML package slot, but like `Onnxify.ML`, it is currently placeholder-level rather than a mature documented surface.
...
- Treat `Onnxify.ML` and `Onnxify.ML.TorchSharp` as names that exist in the outward package surface but are not yet strong sources of behavior unless the code in those projects actually supports the requested feature.
