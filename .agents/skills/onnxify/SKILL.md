...
Read [references/packages.md](references/packages.md) when you need the current `Onnxify.*` NuGet package versions, direct Onnxify package dependencies, or third-party NuGet dependency versions.
...
- `Onnxify.ModelGenerator` (`src/Onnxify.ModelGenerator`): the public Roslyn source-generator package that turns `.onnx` files in a consuming project into typed `Microsoft.ML.OnnxRuntime` wrapper classes with generated input/output contracts, synchronous `Run(...)`, and cancellation-aware `RunAsync(...)` inference overloads.
