...
`Onnxify.TorchSharp` currently references `TorchSharp` `0.107.0`. For local CPU execution, `TorchSharp-cpu` `0.107.0` is the simplest starting point. For GPU execution, install the matching TorchSharp CUDA runtime package for your environment instead.
